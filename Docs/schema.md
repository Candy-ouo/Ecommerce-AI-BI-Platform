...
| r_score | INT | R得分（1=差, 2=中, 3=好） | `3` |
| f_score | INT | F得分（1=低, 2=中, 3=高） | `3` |
| m_score | INT | M得分（1=低, 2=中, 3=高） | `2` |
| rfm_group | STRING | RFM组合编码（3位数字） | `332` |
| rfm_label_cn | STRING | RFM 8类中文标签 | `重要发展用户` |
| dt | STRING | 分区字段（统计截止日） | `2014-12-18` |

**RFM 8类标签对照**（r_score/f_score/m_score 取值均为 1/2/3，共 27 种组合，映射到以下 8 类）：
...
| **重要挽留用户** | 3（高） | 1-2（低） | 1-2（低） | 最近活跃但低频且集中，有流失风险 |
| **一般价值用户** | 1-2（低） | 3（高） | 3（高） | 久未活跃但历史高价值，需召回 |
| **一般发展用户** | 1-2（低） | 3（高） | 1-2（低） | 久未活跃，历史高频但集中 |
| **一般保持用户** | 1-2（低） | 1-2（低） | 3（高） | 久未活跃 + 低频，但买得广 |
| **一般挽留用户** | 1-2（低） | 1-2（低） | 1-2（低） | 三项都低，流失用户 |
